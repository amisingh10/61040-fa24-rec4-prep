...
export default class AuthenticatingConcept {
  ...
  async updateUsername(_id: ObjectId, username: string) {
    ...
    const update = await this.users.partialUpdateOne({ _id }, { username });

    const userUpdate = await this.users.readOne({ _id });
    ...
    return {msg: "Username updated successfully!", user: this.redactPassword(userUpdate)};
  }
